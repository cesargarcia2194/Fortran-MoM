...
call inicializar_MLFMA(meshs%centers, meshs%nedges, e_t, e_po, e_long, t_area, t_baric, t_baric_sub, p_coord, t_p, &
     e_p, num_p, num_t, t_normal)
    ...
    subroutine cap_geom(skipZ)
        ...
        call printconsole( 'Leyendo Mallado...', 'reading_STL')

        meshs = load_mesh_gmsh(conf%file_name)
  ...
  do n=1,meshs%nnodes
    p_coord(:,n) = meshs%nodes(n)%p
    !print*, meshs%edges(i)%bnode_indices
  enddo
  allocate(t_p(3,size(meshs%faces)))
  do n=1,meshs%nfaces
    t_area = meshs%faces(n)%area
    t_baric(:,n) = meshs%faces(n)%cp
    t_p(:,n) = meshs%faces(n)%node_indices
    t_normal(:,n) = meshs%faces(n)%n
  enddo
  ...
  do n=1,meshs%nedges
    e_long = meshs%edges(n)%length
    e_po(:,n) = meshs%edges(n)%bnode_indices
    e_p(:,n) = meshs%edges(n)%node_indices
    e_t(:,n) = meshs%edges(n)%face_indices
  enddo
  !print*, shape(t_p)
  num_e = meshs%nedges
  num_p = meshs%nnodes
  num_t = meshs%nfaces
  e_centro = meshs%centers
